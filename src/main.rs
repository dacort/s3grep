    ...
    let progress = if !opt.quiet {
        ...
        p.set_style(
            ProgressStyle::default_spinner()
                .template("\n{spinner:.green} Processed {pos} files... ({per_sec} files/sec)")?
        );
        Some(p)
    } else {
        None
                        ...
                        match search_object(&client, &bucket, &key, &pattern, case_sensitive).await {
                            Ok(matches) => {
                                for (line_num, line) in matches {
                                    if line_numbers {
                                        println!("s3://{}/{}:{}:{}", bucket, key, line_num, highlight_match(&line, &pattern));
                                    } else {
                                        println!("s3://{}/{}:{}", bucket, key, highlight_match(&line, &pattern));
                                    }
...
fn list_objects_stream<'a>(
    ...
    stream::unfold(
        (client.clone(), bucket.to_string(), prefix.to_string(), Some(String::new())),
        move |(client, bucket, prefix, continuation_token)| async move {
            ...
            match req.send().await {
                Ok(resp) => {
                    let objects: Vec<_> = resp.contents()
                    .iter()
                ...
                Err(e) => {
                    ...
                    let error_stream = empty_vec.into_iter().map(Ok::<String, Box<dyn std::error::Error>>);
                    Some((
                        stream::iter(error_stream),
                        (client, bucket, prefix, None),
                    ))
                }
            }
